...
export interface TemplateValues {
  [k: string]: string | number | TemplateValues;
}
    ...
    keys
      .reduce(
        (result, key, index) => {
          ...
          const value = partialKeys.slice(1).reduce((res, k) => (res as TemplateValues)[k], values[partialKeys[0]]) as string;
...
export class ErrorMessagePipe implements PipeTransform {
  transform(errors: ValidationErrors | null, fieldName?: string): string | undefined {
    ...
    if (templateEntry) {
      ...
      return template({ inputName, error: errors[key] as TemplateValues });
    }
